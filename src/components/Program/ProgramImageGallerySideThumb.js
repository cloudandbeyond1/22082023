...
const ProgramImageGallerySideThumb = ({ product, thumbPosition }) => {
  const [thumbsSwiper, setThumbsSwiper] = useState(null);
  const [index, setIndex] = useState(-1);
  const slides = product?.detailimage.map((img, i) => ({
      ...
      <div className="row row-5 test">
        ...
        >
          <div className="product-large-image-wrapper">
            ...
            {product?.detailimage?.length ? (
              <Swiper options={gallerySwiperParams}>
                {product?.detailimage.map((single, key) => (
                  <SwiperSlide key={key}>
...
ProgramImageGallerySideThumb.propTypes = {
  product: PropTypes.shape({}),
  thumbPosition: PropTypes.string
};

export default ProgramImageGallerySideThumb;
